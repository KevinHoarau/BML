...
def selectedASes(routes, ASes, update):
	...
	if(update["type"]=='A'):
		...
		if(update["peer_asn"] in routes[update["fields"]["prefix"]][update["collector"]] and not routes[update["fields"]["prefix"]][update["collector"]][update["peer_asn"]] is None):
			path = routes[update["fields"]["prefix"]][update["collector"]][update["peer_asn"]].split(" ")
...
class Features(BaseTransform):
	...
	def transforms(self, index, routes, updates):
		...
		for update in updates:	
			...
			if(selectedASes(routes, self.params["ASes"], update)):
